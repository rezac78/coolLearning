...
export default function CourseForm({ initialCourseData, Message, SuccessMessage, Success }: CourseFormProps) {
  ...
  const onSubmit = async (data: any) => {
    try {
      const response = await CourseDataUpdate(initialCourseData._id, data);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-4">
      {InputForm.map((e) => (
        <Inputs value={initialCourseData[e.IdName]} Type="Form" Ekey={e.id} IdName={e.IdName} LabelName={e.LabelName} type={e.type} Register={register} Errors={errors} />
      ))}
      {fields.map((field, index) => (
        <div key={field.id} className="">
          {InputFormChapter.map((e) => (
            <Inputs value={initialCourseData[e.IdName]} index={index} Type="Formatters" Ekey={e.id} IdName={e.IdName} LabelName={e.LabelName} type={e.type} Register={register} Errors={errors} />
          ))}
          <div className="p-4">
